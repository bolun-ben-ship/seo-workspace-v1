...
## Tone Guide (Load Before Writing)

Before starting Phase 1, check for a tone guide in the client's context folder:

```
context/tone-guide.md
```

**If found:** Read it fully. Internalise the voice, do/don't list, sentence style, banned phrases, and CTA rules. These override default writing style for every part of this post.

**If absent:** Note "No tone guide found — using default style + any voice/tone cues from client-info.md." Check `context/client-info.md` for any Voice & Tone section as a fallback.

The tone guide applies to: all body copy, headings, CTAs, FAQ answers, TL;DR box, and the intro. It does NOT override keyword placement or structural SEO rules.
...
```bash
mkdir -p "Content & SEO/outputs/<domain>/blogs"
```

Save each post to: `Content & SEO/outputs/<domain>/blogs/<post-slug>.html`

Before writing any post, check `Content & SEO/outputs/<domain>/blogs/` and
`Content & SEO/outputs/<domain>/blog-plans/` for existing posts and plans.
Do NOT write a post that duplicates or significantly overlaps a topic already written.
If a similar post exists, flag it to the user and suggest a differentiated angle instead.
