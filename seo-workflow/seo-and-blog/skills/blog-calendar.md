...
```bash
mkdir -p "Content & SEO/outputs/<domain>/blog-plans"
```

Save the calendar to: `Content & SEO/outputs/<domain>/blog-plans/BLOG-PLAN-YYYY-MM-DD.md`
(use today's date — never overwrite prior plans).

Before writing the plan, check `Content & SEO/outputs/<domain>/blog-plans/` for any previous
`BLOG-PLAN-YYYY-MM-DD.md` files. Load the most recent and do NOT plan posts with the same
topic, title, or keyword cluster as posts already planned or written. Avoid cannibalisation.
